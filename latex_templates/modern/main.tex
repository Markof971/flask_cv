...
{% for job in jobs %}
\textsc{{ job.title }} at \textit{{ job.company }}{% if job.location %} ({{ job.location }}){% endif %}  \dates{{ job.start_date }}--{{ job.end_date }} \\
{% for line in job.bullets.splitlines() if line.strip() %}
\smaller{{ line }}\is
{% endfor %}
{% endfor %}
